    ...
    return scrapeIt('http://beervana.sk/', {
        ...
        beer: {
            ...
            data: {
                ...
                remaining: {
                    selector: 'div.remain-txt span:not(.newtxt):not(.hurrytxt)',
                    how: 'text',
                    convert: x => x.replace(/Nezmeškaj/gi, '').replace(/Novinka/gi, '')
                },
                description: {
                    selector: 'div.share-open div.center-box p:nth-of-type(2)',
                    how: 'text'
                },
                rateBeerLink:{
                    selector: 'div.share-open div.center-box a',
                    attr: "href"
                },
                beerStyle: {
        ...
        if (page.data && page.data.beer) {
            for (let oneBeer of page.data.beer) {
                if (oneBeer.beerStyle) {
                    let foundStyle = page.data.beerStyles.find((style) => {
